...
export async function saveFaviconAsAsset(url, domain, insertId) {
  ...
  try {
    ...
    console.log("RESPONSE URI FROM FAVICON Request: ", response.uri);
    ...
    db.transaction((tx) => {
      ...
      tx.executeSql('UPDATE web SET favicon = ? WHERE id = ?', [fileInfo, insertId]);
    });
...
async function fetchFavicon(url) {
  ...
  return updatedFileInfo; // Return the updatedFileInfo object
}
